...
export default function PastQuizPage() {
    ...
    useEffect(() => {
        fetch(`/api/v1/takepastquiz?quiz_id=${quizId}`)
            .then((r) => {
    ...
    if (loading) return <div className="min-h-screen flex items-center justify-center bg-[#f5f3ef]"><p className="text-[#94a3b8]">Loading…</p></div>;
    ...
    if (!quiz || attemptId === null) return null;
    ...
    async function saveAnswer(ans: Answers[number]) {
        setAnswers((prev) => ({ ...prev, [question.question_id]: ans }));
        await fetch("/api/v1/submitanswer", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ attempt_id: attemptId, question_id: question.question_id, ...ans }),
        });
    }

    function selectSCQ(option_id: number) {
        saveAnswer({ option_id });
    }
    ...
    function toggleMCQ(option_id: number) {
        ...
        saveAnswer({ selected_option_ids: next });
    }

    async function handleSubmit() {
        setSubmitting(true);
        const res = await fetch("/api/v1/pasttestsubmit", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
        ...
        if (res.ok) {
            router.push(`/QuizResult?attempt_id=${attemptId}`);
        } else {
            ...
            <div className="min-h-screen bg-[#f5f3ef] flex items-center justify-center p-6">
                <div className="w-full max-w-xl bg-white rounded-3xl shadow-sm overflow-hidden">
                    ...
                    <div className="flex items-start justify-between gap-4 px-6 pt-6 pb-4">
                        ...
                        <div className="shrink-0 border border-[#f1ede8] rounded-2xl px-4 py-2 text-center">
                            <p className="text-xl font-black text-[#f27f0d]">+{question.correct_points}</p>
                            <p className="text-[9px] font-bold text-[#94a3b8] tracking-widest uppercase">Points</p>
                        </div>
                    </div>
                    ...
                    <div className="px-6 pt-5 pb-3">
                        ...
                        <p className="mt-1 text-xs text-[#94a3b8] italic">
                            {question.question_type === 1 && "Select one correct answer."}
                            {question.question_type === 2 && "Select all correct answers."}
                            {question.question_type === 3 && "Enter the correct integer."}
                        </p>
                    </div>
                    ...
                    <div className="px-6 flex flex-col gap-2.5 pb-4">
                        {question.question_type === 3 ? (
                            <input
                                ...
                                onChange={(e) => saveAnswer({ integer_answer: parseInt(e.target.value, 10) || undefined })}
                                className="border border-[#e2e8f0] rounded-xl px-4 py-3 text-sm font-semibold outline-none focus:border-[#f27f0d]"
                            />
                        ) : (
                            question.options.map((opt, idx) => {
                                ...
                                return (
                                    <button
                                        key={opt.option_id}
                                        onClick={() => question.question_type === 1 ? selectSCQ(opt.option_id) : toggleMCQ(opt.option_id)}
                    ...
                    <div className="flex items-center justify-between px-6 py-5 border-t border-[#f1ede8]">
                        ...
                        {currentIndex < total - 1 ? (
                            ...
                            <button
                                ...
                                disabled={submitting}
                                className="flex items-center gap-2 h-11 px-6 rounded-xl bg-[#f27f0d] text-white text-sm font-bold hover:bg-[#e0720a] transition-colors cursor-pointer disabled:opacity-50"
                            >
